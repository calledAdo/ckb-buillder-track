...
use ckb_std::{
    ...
    high_level::{
        load_cell_data, load_cell_type_hash, load_input, load_script, load_tx_hash, load_witness,
        QueryIter,
    },
};
...
pub fn check_signature_from_witnesses(expected_pubkey_hash: &[u8]) -> Result<(), i8> {
    ...
    #[cfg(not(test))]
    {
        ...
        auth.pubkey_hash
            .copy_from_slice(&expected_pubkey_hash[0..20]);
...
fn generate_sighash_all() -> Result<([u8; 32], Bytes), i8> {
    ...
    let witness_args =
        WitnessArgs::from_slice(&witness_bytes).map_err(|_| ERROR_WITNESS_MISSING)?;
    ...
    let zero_lock = ckb_std::ckb_types::packed::Bytes::new_builder()
        .set(vec![
            ckb_std::ckb_types::packed::Byte::new(0);
            signature_bytes.len()
        ])
        .build();
    ...
    let modified_witness_args = witness_args.as_builder().lock(lock_opt).build();
